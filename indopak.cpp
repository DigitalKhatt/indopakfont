...
void IndoPak::generateGlyphs() {
  ...
  // addFake("yehfarsi.isol", 0x06CC, m_layout->glyphNamePerCode.rbegin()->first + 1);
...
IndoPak::IndoPak(OtLayout* layout, MPFont* font, bool extended) : Automedina{layout, font, extended} {
  ...
  classes["waqfmarksfina"].erase("sajda");
  ...
  layout->expandableGlyphs["hah.medi.aftermeem.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["hah.medi.afterfeh.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["seen.medi.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["sad.medi.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["tah.medi.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["ain.medi.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["fehshape.medi.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["kaf.medi.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["lam.medi.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["am.medi.afterkaf.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["meem.medi.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["meem.medi.afterhah.besad"] = {0, 0, 20, 0};
  layout->expandableGlyphs["kaf.medi.ii.besad"] = {0, 0, 20, 0};
}

CursiveAnchorFunc IndoPak::getCursiveFunctions(std::string functionName, Subtable* subtable) {
...
CalcAnchor IndoPak::getanchorCalcFunctions(std::string functionName, Subtable* subtable) {
  CalcAnchor tret;
...
Lookup* IndoPak::getLookup(std::string lookupName) {
...
Lookup* IndoPak::allCursiveJoin(bool rtl) {
  ...
  for (auto& [cursiveName, entries] : entryAnchorsRTL) {
  ...
  for (auto& [cursiveName, entries] : entryAnchors) {
...
Lookup* IndoPak::defaultmarkposition() {
  ...
  topmarks.erase("meemiqlab");

  auto lowmarks = classes["lowmarks"];
  lowmarks.erase("hamzabelow");
  lowmarks.erase("hamzaabove.joined");
...
Lookup* IndoPak::waqfMkmkPositioning() {
  ...
  auto addLookup = [this, &waqfCodes, &endWordCodes, &endWordClass](int numMarks) -> void {
    ...
    for (auto& seq : waqfSequences) {
      ...
      for (std::uint16_t i = 2; i <= numMarks; i++) {
        subtable->compiledRule.input.push_back({seq[i - 1]});
        subtable->compiledRule.lookupRecords.push_back({i, "waqfmkmkpositioning.lmkmk"});
      }
      ...
      int waqfHeight = seq.size() == 1 ? 200 : seq.size() == 2 ? 300
                                           : seq.size() == 3   ? 400
                                                               : 500;
  ...
  subtable->compiledRule.lookupRecords.push_back({ 0,asStdString(QString("l4") )});
  addLookup(4);
  */
  ...
  subtable->compiledRule.lookupRecords.push_back({ 0,asStdString(QString("l3") )});
  addLookup(3);*/
...
Lookup* IndoPak::glyphalternates() {
  ...
  altfeatures.push_back({"cv12", {{"behshape.init.beforehah", "behshape.init"}, {"hah.medi.afterbeh", "hah.medi"}, {"hah.medi.afterbeh.besad", "hah.medi.besad"}, {"hah.medi.afterbeh.beforeyeh", "hah.medi.beforeyeh"}}});
  altfeatures.push_back({"cv13", {{"meem.init.beforehah", "meem.init"}, {"hah.medi.aftermeem", "hah.medi"}, {"hah.medi.aftermeem.besad", "hah.medi.besad"}}});
  altfeatures.push_back({"cv14", {{"fehshape.init.beforehah", "fehshape.init"}, {"hah.medi.afterfeh", "hah.medi"}, {"hah.medi.afterfeh.besad", "hah.medi.besad"}}});
  altfeatures.push_back({"cv15", {{"lam.init.lam_hah", "lam.init"}, {"hah.medi.lam_hah", "hah.medi"}, {"hah.medi.lam_hah.besad", "hah.medi.besad"}}});
  ...
  altfeatures.push_back({"cv18", {{"hah.init.beforemeem", "hah.init"}, {"meem.medi.afterhah", "meem.medi"}, {"meem.medi.afterhah.besad", "meem.medi.besad"}}});
  ...
  for (auto mapping : cv01mappings) {
    ...
    if (sameSubst) {
      alternates.push_back({substcode, 12, 0});
    }
    alternateSubtable->alternates[code] = alternates;
  }
